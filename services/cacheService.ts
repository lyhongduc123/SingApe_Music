...
export async function getListeningHistory(): Promise<
  Array<{ track: MyTrack; timestamp: string }>
> {
  try {
...
export async function deleteListeningHistory(trackId: string) {
  ...
  if (fileInfo.exists) {
    ...
    const updatedHistory = history.filter(
      (item: any) => item.track.id !== trackId
    );
...
const getPlaylistFilePath = (playlistId: string) =>
  `${PLAYLISTS_DIR}${playlistId}.json`;
...
export const createPlaylist = async (
  ...
  await FileSystem.writeAsStringAsync(filePath, JSON.stringify(newPlaylist));
  return newPlaylist;
};
...
export const addSongToPlaylist = async (id: string, song: MyTrack) => {
  const filePath = getPlaylistFilePath(id);
  const fileInfo = await FileSystem.getInfoAsync(filePath);
  if (!fileInfo.exists) {
    throw new Error("Playlist does not exist");
  ...
  const existingTrack = playlist.tracks.find(
    (track: MyTrack) => track.id === song.id
  );
  if (existingTrack) {
...
export const removeSongFromPlaylist = async (id: string, song: MyTrack) => {
  const filePath = getPlaylistFilePath(id);
  const fileInfo = await FileSystem.getInfoAsync(filePath);
  if (!fileInfo.exists) {
    throw new Error("Playlist does not exist");
...
export const addSongToFavorite = async (song: MyTrack) => {
  ...
  const existingTrack = favorites.find(
    (track: MyTrack) => track.id === song.id
  );
  if (existingTrack) {
...
export const checkIfSongInFavorites = async (
  song: MyTrack
): Promise<boolean> => {
  const filePath = getFavoriteFilePath();
  const fileInfo = await FileSystem.getInfoAsync(filePath);
...
export const getRecentSearch = async () => {
  try {
    ...
    console.log("Error reading recent search:", e);
    return [];
  }
};
